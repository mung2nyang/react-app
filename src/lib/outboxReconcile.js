...
/**
 * @template {{ supabaseId?: string|number }} T
 * @param {string} ownerKey
 * @param {Array<T>} mergedCars
 * @returns {Array<T>}
 */
export function reconcileCars(ownerKey, mergedCars) {
  return mergedCars.filter((car) => !isTombstoned(ownerKey, 'vehicle', String(car.supabaseId || '')))
}

/**
 * @template {{ supabaseId?: string|number }} T
 * @param {string} ownerKey
 * @param {Array<T>} mergedClients
 * @returns {Array<T>}
 */
export function reconcileClients(ownerKey, mergedClients) {
  return mergedClients.filter((client) => !isTombstoned(ownerKey, 'client', String(client.supabaseId || '')))
}

/**
 * @param {DriverRecord} driver
 * @param {OutboxOp} pending
...
/**
 * @param {string} ownerKey
 * @param {Array<DriverRecord>} mergedDrivers 서버 응답과 병합된(=hydrateMerge.mergeDriversFromRows 결과) 목록
 * @param {Array<Partial<DriverRecord>>} localDrivers 병합 전 로컬 목록 — 서버에 아직 없는 pending 생성 건을 되찾는 데 쓴다
...
export function reconcileDrivers(ownerKey, mergedDrivers, localDrivers) {
  ...
  return [...reapplied, .../** @type {Array<DriverRecord>} */ (recovered)]
}
